...
function interpAt(profile: number[], u: number): number {
...
function Tube({ texture, position }: { texture: THREE.Texture; position: Vec3 }) {
  return (
    <mesh position={position} rotation={[0, 0, Math.PI / 2]}>
      ...
      <meshStandardMaterial map={texture} side={THREE.DoubleSide} roughness={0.35} metalness={0.1} />
    </mesh>
  )
}
...
/** 3D PFR vessel: 7-tube hex shell-and-tube bundle, each tube surface textured with
 * an axial conversion gradient, flowing particles colored by local conversion —
...
export function PFRScene({ xaProfile, flowFraction }: PFRSceneProps) {
  ...
  return (
    <Canvas camera={{ position: [3.6, 1.6, 3.6], fov: 38 }} style={{ width: '100%', height: '320px' }}>
      <ambientLight intensity={0.7} />
      <pointLight position={[5, 5, 5]} intensity={80} />
      <pointLight position={[-5, -2, -5]} intensity={20} />
      <Header x={-(TUBE_LENGTH / 2 + HEADER_LENGTH / 2)} />
      <Header x={TUBE_LENGTH / 2 + HEADER_LENGTH / 2} />
